...
public class FogBroker extends PowerDatacenterBroker{
    ...
    //rajouté des 4 lignes suivantes par Edner
    public static final int WOA_INIT = 101;
    public static final int WOA_UPDATE = 102;
    public static final int WOA_GBEST = 103;

    private int woaState = WOA_INIT;

    public void setWoaState(int state) {
        this.woaState = state;
    }
        ...
        switch (ev.getTag()) {
            ...
            case WorkflowSimTags.CLOUDLET_UPDATE:
				...
				case GA:
					...
					break;
				case WOA: // rajout edner
				    if (woaState == WOA_INIT) {
				        processCloudletUpdateForWOAInit(ev);
				    } else if (woaState == WOA_UPDATE) {
				        processCloudletUpdateForWOAUpdate(ev);
				    } else { // WOA_GBEST
...
//            System.out.println(cloudlet.getCloudletId()+".submit delay : "+delay);
    ...
    // rajouter par edner (ligne 335-448)
    protected void processCloudletUpdateForWOAInit(SimEvent ev) {
        List<Cloudlet> cloudletList = getCloudletList();

        WorkflowEngine engine = (WorkflowEngine) CloudSim.getEntity(workflowEngineId);
        double[] schedule = engine.getWoaCurrentSchedule(); // whale courante
        ...
        for (int i = 0; i < cloudletList.size(); i++) {
            ...
            int vmId = (int) schedule[cloudletId]; // comme PSO: schedule[taskId]
            // si tu veux respecter l'offloading comme GA :
            vmId = ChooseVm(cloudletList.get(i), vmId);
    ...
    protected void processCloudletUpdateForWOAUpdate(SimEvent ev) {
        List<Cloudlet> cloudletList = getCloudletList();

        WorkflowEngine engine = (WorkflowEngine) CloudSim.getEntity(workflowEngineId);
        double[] schedule = engine.getWoaCurrentSchedule(); // whale courante (après update)
    ...
    protected void processCloudletUpdateForWOAGbest(SimEvent ev) {
        List<Cloudlet> cloudletList = getCloudletList();

        WorkflowEngine engine = (WorkflowEngine) CloudSim.getEntity(workflowEngineId);
        double[] schedule = engine.getWoaBestSchedule(); // best whale globale
